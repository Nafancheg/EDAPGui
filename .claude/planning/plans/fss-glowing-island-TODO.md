...
- [x] 🟣 **РЕВИЗИЯ EDMesg СДЕЛАНА (2026-07-07).** Вердикт: (1) **транспорт** — EDMesg это **ZeroMQ** (`import zmq`, PUSH/PULL+PUB/SUB), браузер по нему говорить не может → **не годится как транспорт веб-MCDU**; HTTP+WebSocket (сделан в 7.2) обязателен независимо. (2) **`ap_ckb`→событийный поток формализован в 7.2** (`Broadcaster`). (3) **⚠️ НАХОДКА — EDMesg частично сломан после Фазы 1:** `EDAP_EDMesg_Server.py` дёргает удалённые `self.ap.waypoint`/`system_map`/`galaxy_map`/`tce_integration` (waypoint/карты/TCE-действия → `AttributeError` при вызове). Не падает только т.к. `EnableEDMesg=False` по умолчанию (сервер не стартует). Живые действия: `GetEDAPLocation`, `StopAllAssists` (через `ap_ckb`), `Launch`. **Решение → EDMesg УДАЛЁН ЦЕЛИКОМ** (2026-07-07, решение пользователя «мёртвый груз, мочи»): раз транспорт для браузера не годится (ZeroMQ), а канал выключен по умолчанию и частично сломан — вырезан полностью (`EDAP_EDMesg_Server/Client/Interface.py` + `EDMesg/` + инстанс `mesg_server` + конфиг-ключи `EnableEDMesg`/`EDMesgActionsPort`/`EDMesgEventsPort` + зависимость `pyzmq`). QA: 0 ссылок в репо, import чист, GUI стартует. Понадобится внешнее управление — сделаем bespoke под конкретную нужду. Веб-контракт (`webserver/`) самодостаточен. См. [[web-ui-direction]].
...
>
> **🎨 ДИЗАЙН-РЕФЕРЕНС ЕСТЬ (обновлён 2026-07-09):** `design/web-mockup/EDAutopilot-MCDU.dc.html` — интерактивный `.dc`-прототип вертикального прибора A320 MCDU (LSK-семантика со скретчпадом уже в макете: непустой скретчпад → LSK вводит значение в поле, пустой → действие поля; функциональные клавиши, клавиатура A–Z). Палитра: фон `#08080a`, янтарь `#E8973E`, заголовок `#5f96d6`, действия `#5aa9c9`; шрифты IBM Plex Mono (корпус) + VT323 (ЭЛТ). Старый горизонтальный дашборд (`EDAutopilot.dc.html`) удалён 2026-07-09 — есть в git-истории; из него надо перенести содержимое страницы 2/2 (RPY-тюнинг/кривые), в новом макете нарисована только 1/2. ЭТО ВИЗУАЛЬНЫЙ ТАРГЕТ, но `.dc`+support.js в Python НЕ встроен → 7.3 = собрать **self-contained production-фронтенд** по этому дизайну, подключённый к WS-контракту `webserver/`, заменив заглушку `webserver/static/index.html`.
- [x] 🟣 [INLINE] Каркас MCDU: **один** экран с page-flip, 6 рабочих LSK/сторону, компактный скретчпад, без дублирования SHIP/статус-бара. Тема янтарь/зелёный, моноширинный, dot-leaders — сохранить. Адаптив под планшет (touch-таргеты ≥44px). *(СДЕЛАНО 2026-07-09: `webserver/static/{index.html,mcdu.css,mcdu.js}` — вертикальный прибор по дизайн-таргету, страницы INIT/ROUTE/FUEL + постоянная панель MESSAGE LOG под прибором (решение пользователя: экранные цифро/буквенные клавиатуры убраны, на их месте лог; скретчпад — с физической клавиатуры, DATA-клавиша INOP), LSK-семантика со скретчпадом (ввод значения/действие/NOT ALLOWED/PAGE INOP), функц. клавиши INIT/F-PLN/FUEL PRED, slew-пагинация, WS-клиент с reconnect; touch ≥44px, mobile-адаптив без h-scroll. Серверу добавлены команды `config.set` (+broadcast конфига) и `route.get` (маппинг NavRoute → системы с флагом scoopable KGBFOAM + dist_ly). QA: in-process 22/22 PASS + приёмка в реальном браузере на fake-ядре.)*
- [ ] 🟣 [INLINE] **ROUTE-страница** (эталонный сценарий пользователя): список систем маршрута, каждая с флагом заправляемости (✗ красным для не-заправляемых). LSK на строке → подменю (RSS-скан / инфо о теле / искать заправку в радиусе). LSK «SCOOP SEARCH» → радиус из скретчпада → страница перестраивается в список ближайших заправляемых, каждая с LSK «проложить через неё». Данные — из `navigation_service`/`NavRouteParser`/`FuelState`. *(ЧАСТИЧНО 2026-07-09: список систем с флагами ✗/SCOOP, дистанциями, ORIGIN/DEST, подсветкой текущей системы и пагинацией — готов и принят в браузере; LSK на строке пока даёт инфо-строку (класс/заправляемость). ОТЛОЖЕНО: подменю строки и SCOOP SEARCH — «проложить через систему» требует автоматизации галактической карты (EDGalaxyMap удалён в Фазе 1) и внешнего источника соседних систем (EDSM/Spansh) — отдельное решение.)*
- [ ] 🟣 [INLINE] **FUEL-страница**: `FuelState` (бак, прогноз на N прыжков, где следующая заправка) — витрина Фазы 3. *(Заглушка готова 2026-07-09: бак % + текущая звезда + «AWAIT FUELSTATE (PHASE 3)»; прогноз — после Фазы 3.)*
- [ ] 🟣 [INLINE] Прогрессивно по страницам: сначала ROUTE + LOG + базовые тумблеры (FSD/SC/Fast Travel), потом FUEL/SHIP/PROG/NAV/CALIBRATION. *(Первая ступень СДЕЛАНА 2026-07-09: ROUTE + LOG (DATA) + тумблеры FSD/SC/Fast Travel + throttle + STOP ALL. Дальше: SHIP (RPY-тюнинг — перенести содержимое страницы 2/2 старого макета), PROG, NAV, CALIBRATION.)*
