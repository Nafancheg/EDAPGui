...
- **`EDMesg` REMOVED entirely (2026-07-07, user decision).** It was kept in Phase 1 as a *possible* web-server foundation, but that turned out moot: it is a **ZeroMQ** IPC channel and browsers can't speak ZeroMQ (the web UI uses HTTP+WebSocket, built in 7.2), and its handlers still referenced Phase-1-deleted peripherals (waypoint/galaxy_map/system_map/tce). Off by default and unused → deleted wholesale (EDAP_EDMesg_Server/Client/Interface.py + EDMesg/ dir + the mesg_server instance + EnableEDMesg/EDMesgActionsPort/EDMesgEventsPort config keys + the pyzmq dep). If external control is ever needed again, build a bespoke channel for the concrete need. **Still valid:** the `ap_ckb` callback bridge is the core↔UI seam — it was formalized into the web event stream by the `Broadcaster` in 7.2, not rebuilt from scratch.
