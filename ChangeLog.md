# 7/5/2026 v1.11.0 (fork)
Авто-FSS, часть B (скан одного тела за нажатие):
- Новая кнопка СКАН1 на мини-панели: наводит камеру FSS на ближайшую к центру кляксу, зумит и ждёт подтверждения скана по журналу (событие Scan), затем возвращает зум. Одно тело за нажатие.
- Наведение самообучающееся: направление осей (в т.ч. инверсия питча) и скорость камеры (сек. удержания клавиши на долю экрана) выучиваются по фактическому движению кляксы между кадрами — предварительная калибровка не нужна.
- Параметры в конфиге (без GUI): AutoFSSCenterTol (0.02), AutoFSSMaxCenterIters (14), AutoFSSMaxZoomSteps (2), AutoFSSScanTimeout (8 с).
- Защиты: проверка GuiFocus=FSS на каждой итерации, guard от повторного запуска, при таймауте скана зум возвращается назад.
- В .binds пользователя прописаны клавиатурные бинды FSS на нумпад (камера 8/2/4/6, зум */и/, точный зум 9/7, тюнер 3/1) — вторым слотом, джойстик не тронут; бэкап создан.

Авто-FSS, часть A (подготовка, без сканирования):
- EDKeys читает биндинги FSS: зум (ExplorationFSSZoomIn/Out, MiniZoomIn/Out), шаги камеры (CameraPitch/Yaw Increase/DecreaseButton), тюнер (RadioTuningX_Increase/Decrease).
- Новый CV-детектор нерезолвленных сигналов fss_find_signal_blobs: поиск бледных «клякс» по насыщенности/яркости (HSV), с отсевом оранжевого HUD, засветки звезды и полосы спектра; порог настраивается (AutoFSSBlobThreshold, по умолчанию 48).
- Кнопка АFSS на мини-панели — сухой прогон: открывает FSS (если закрыт), находит кляксы, сохраняет размеченный скриншот в debug-output/images (afss_blobs_*.png), пишет координаты в лог. Ничего не сканирует и не жмёт — инструмент калибровки перед частью B.
- При запуске теста сообщается, каких клавиатурных биндов не хватает для будущего авто-сканирования.
- Части B (наведение камеры + зум + подтверждение по журналу) и C (полный цикл, свип, интеграция в маршрут для неоткрытых систем) — следующими сессиями.

FSS-советчик (по журналу, только текст — без голоса):
- EDJournal теперь парсит события разведки: Scan, FSSBodySignals, FSSDiscoveryScan (гудок), FSSAllBodiesFound; состояние сбрасывается при FSDJump.
- При ручном FSS-сканировании автопилот мгновенно пишет вердикт по каждому ценному телу (ELW/WW/AW, терраформируемые, био/гео сигналы): «НЕ открыта — первооткрытие!» / «открыта, но не картографирована — бонус за карту» / «уже открыта и картографирована — пропускай».
- Оверлей и мини-панель: строка прогресса «FSS СКАН: X/Y (ГОТОВО)» и до 4 последних ценных тел с маркерами [1st!]/[map$]/[known] (подсвечены зелёным).
- Сообщения по гудку: «в системе N тел» / «сканировать нечего, всё уже найдено» (Progress=1.0, например система из одной звезды); по завершении — «система просканирована полностью, ценных тел: N».
- Вердикт задерживается на ~1 с, чтобы FSSBodySignals успел слиться со Scan независимо от порядка событий; повторный запуск приложения не переобъявляет старые сканы (гашение реплея журнала).
- EDSM-проверка расширена: для ценных тел из базы выводится имя, первооткрыватель и дата; вердикт «полностью известна» либо «N тел нет в базе EDSM».
- Примечание: «первый шаг» (first footfall) недоступен ни в журнале, ни в публичных API — ориентир ценности это WasMapped (бонус первой картографии).

# 7/4/2026 v1.10.0 (fork)
Наведение и выравнивание:
- Исправлена мёртвая проверка «цель позади» в sc_target_align (bool сравнивался с < 0 и никогда не срабатывал) — устраняет бесконечное кружение при уходе цели за корму.
- ML-детекция (компас/цель) теперь берёт бокс с максимальной уверенностью по каждому классу вместо последнего — ложные срабатывания больше не затирают настоящую цель.
- Добавлен лимит (2) на репозиции при окклюзии цели за один вызов выравнивания — устраняет вечный цикл «пике-подъём» при наведении на планету.
- Добавлен таймаут 60 с на цикл выравнивания sc_target_align.
- Ослабление допусков ×3 при фолбэке на компас теперь применяется и внутри цикла выравнивания.
- Новые параметры jump_align_outer_lim/jump_align_inner_lim (3.0/2.0 град.) — перед гиперпрыжком не требуется субградусное наведение (FSD сам центрует в своём конусе), убирает раскачку.
- Защита экстраполяции RPY-кривых от нулевой/отрицательной скорости.
- Auto-tune RPY отбрасывает неправдоподобные замеры (шум детекции), не отравляя кривые.
- compass_align: холостые continue заменены на break при потере компаса.

Перегрев и заправка:
- Новый аварийный манёвр overheat_escape: газ 100%, отворот от звезды, ожидание остывания.
- Проверка флага перегрева (Status.json) в sc_target_align, compass_align, position(), на подходе к заправке и в обоих циклах скупа — при перегреве текущая операция прерывается и корабль отворачивает.
- sun_avoid больше никогда не опускает нос к звезде при уже идущем перегреве.
- Профиль корабля mandalay в ship_configs.json: SunPitchUp+Time = 1.0 (дефолт -1.0 вёл горячий корабль прямо в звезду при заправке).

Сканер ELW:
- Исправлен маппинг зон спектра FSS (реальный порядок: Water → Earth-like → Ammonia; был перепутан и противоречил собственному комментарию).
- Перед детекцией проверяется, что FSS реально открылся (GuiFocus) — устраняет ложные «Earth» по скриншоту кабины.
- Дебаг-скриншот теперь сохраняет саму полоску спектра (раньше — бесполезный угол экрана с чатом).
- В elw.txt пишется позиция совпадения (MatchX) для калибровки.

EDSM и оверлей:
- После каждого прыжка фоновый запрос к EDSM: системы нет в базе → «возможно неоткрытая» (зелёная подсветка + голос); иначе — число тел, сколько не открыто, первооткрыватель, ценные тела (ELW/WW/AW/терраформ.). Опция EDSMCheckEnable.
- Оверлей: добавлены топливо, скупаемость звезды, цель с остатком прыжков, дистанция, строка EDSM.
- Общий метод get_status_lines() для оверлея и мини-панели; неиспользуемые строки оверлея очищаются.

Интерфейс:
- Мини-панель поверх игры (always-on-top, перетаскиваемая): переключатели FSD/SC/WP, кнопка СТОП, переключатель БЫСТРО и полный блок статуса как в оверлее (обновление 1 раз/с). Кнопка вызова — в блоке MODE.
- Режим «Быстрый перелёт» (FastTravelMode): пропуск honk, FSS/ELW-сканирования и паузы на остывание (если нет перегрева).
- Локализация всех новых строк (оверлей, голос, сообщения) через locales/*.json; ключи добавлены во все 5 языков.

Прочее:
- Логи перенесены в папку logs/, автоматическая миграция из корня, хранятся 10 последних архивов.
